...
public class EditingCameraMovement : MonoBehaviour {
	...
	public float zoomSensitivity	= .007f;
	...
	void Awake() {
		...
		Conatrols.IM.Editing_Camera.FocusMouse.Subscribe<Contexts.Editing>(FocusMouse);
		Conatrols.IM.Editing_Camera.FocusSelection.Subscribe<Contexts.Editing>(FocusSelection);
	}
	...
	void Update() {
		...
		globalSensitivity = Conatrols.IM.Editing_Camera.Precision.IsPressed() ? precisionCoef : 1;

		// orbit
		vel *= orbitDrift;
		if (Conatrols.IM.Editing_Camera.PerfOrbit.IsPressed()) {
			Orbit();
			Movement();
		}

		// move todo: sensitivity changes with screen resolution. no good fix. bad fix for now. please fix future me. sorry.
		else if (Conatrols.IM.Editing_Camera.Move.IsPressed())
			target += Camera.transform.rotation * -Conatrols.Mouse.Delta * moveSensitivity * Mathf.Abs(dist) * globalSensitivity; // / Screen.width * 1920;
		...
		Camera.transform.SetPositionAndRotation(r * Vector3.forward * dist + focus, r);
	}
	void Orbit() {
	...
	void Movement() {
		Vector3 movement = Conatrols.IM.Editing_Camera.KeyboardMovement.ReadValue<Vector3>();
		Vector3 globalMove = Camera.transform.rotation * movement;
		target += globalMove * keyboardmoveSpeed;
	}
	void Zoom() {
		targetDist += Conatrols.IM.Editing_Camera.Zoom.ReadValue<float>() * zoomSensitivity * globalSensitivity;
